...
### Manual Review Steps with Rich Content

Manual steps automatically get a review document when the previous step returns content in the right shape. The `autoAttachDocumentFromInput` mechanism creates and links a document to the manual task.

**To generate a review document from a code step**, return an object with `result.content`:

```javascript
return {
  result: {
    title: 'Review: My Workflow',     // Document title
    content: '# Markdown content...',  // Markdown body (supports mermaid diagrams)
    summary: 'Brief summary text',     // Shown on task card and document header
  },
  // Additional fields surface as linked assets in the UI:
  workflowId: 'abc123',     // Links to workflow definition in Produced Assets
  workflowName: 'My Flow',  // Display name for the link
};
```

The document is created with `status: 'review'` and `metadata.autoGenerated: true`, then rendered inline in the ManualReviewPanel with full markdown support (including mermaid diagrams via ` ```mermaid ` code blocks).

**Linkable fields for Produced Assets panel**: Any of these in the code step return value (at `output.*` or `output.result.*`) create clickable links in the task UI:

| Field | Asset Type | Links To |
|-------|-----------|----------|
| `workflowId` | Workflow | `/workflows?workflowId=...` |
| `workflowName` | — | Display name for workflowId link |
| `externalUrl` | External | The URL directly |
| `externalTitle` | — | Display name for externalUrl link |

Manual tasks also automatically show a "Part of" link to their parent workflow run.

### Code Step API Access

Code steps can call the platform API using `node-fetch` and system variables:

```javascript
// In codeConfig:
{
  code: `
    const res = await fetch(apiUrl + '/workflows/' + someId, {
...
Available system variables: `_apiUrl` (API base URL), `_apiKey` (system API key). Additional variables can be mapped from `trigger.*`, `input.*`, `steps.*`, or `variables.*` (variable packages).
