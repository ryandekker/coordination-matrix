...
import { loadPackageContext, resolveTitleTemplate, resolveAgentTemplate, resolveAllAgents } from './template-utils.js';
...
describe('resolveAllAgents', () => {
  it('should return empty array for single criteria with no DB', async () => {
    const result = await resolveAllAgents('complexity.3');
    expect(result).toEqual([]);
  });

  it('should return empty array for combined criteria with no DB', async () => {
    const result = await resolveAllAgents('complexity.3+tag.reasoning');
    expect(result).toEqual([]);
  });

  it('should return empty array for empty query', async () => {
    const result = await resolveAllAgents('');
    expect(result).toEqual([]);
  });

  it('should parse single criteria correctly', async () => {
    const result = await resolveAllAgents('tag.api-integration');
    expect(Array.isArray(result)).toBe(true);
  });

  it('should parse combined criteria with + separator', async () => {
    // Verify it handles the + separator without errors
    const result = await resolveAllAgents('complexity.2+tag.content-writing');
    expect(Array.isArray(result)).toBe(true);
  });

  it('should handle whitespace in + separated criteria', async () => {
    const result = await resolveAllAgents('complexity.3 + tag.reasoning');
    expect(Array.isArray(result)).toBe(true);
  });
});
