    ...
    try {
      ...
      const eventData = {
        ...
        task: event.task ? {
          ...
          extraPrompt: event.task.extraPrompt,
          ...
          workflowStage: event.task.workflowStage,
          taskType: event.task.taskType,
          batchCounters: event.task.batchCounters,
          metadata: event.task.metadata,
          taskResult: event.task.taskResult,
          tags: event.task.tags || [],
          dueAt: event.task.dueAt,
          stepConfig: event.task.stepConfig,
          webhookConfig: event.task.webhookConfig,
          reviewDecision: event.task.reviewDecision,
          reviewComment: event.task.reviewComment,
          reviewedAt: event.task.reviewedAt,
