...
export const schemas = {
  ...
  Workflow: {
    ...
    properties: {
      ...
      rootTaskTitleTemplate: { type: 'string', description: 'Template for root task title, supports {{input.field}} variables' },
      samplePayload: { type: 'string', description: 'Sample JSON payload for triggering this workflow' },
      inputSchema: { type: 'object', description: 'Optional JSON Schema to validate inputPayload when starting workflow runs' },
      createdAt: { type: 'string', format: 'date-time' },
      updatedAt: { type: 'string', format: 'date-time' },
      createdById: { $ref: '#/components/schemas/ObjectId', nullable: true },
    },
